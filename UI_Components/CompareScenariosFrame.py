...
class CompareScenariosFrame(LabelFrame):
    def __init__(self, master,console_frame, params, scenario_data, plot_frame, *args, **kwargs):
        ...
        self.scenario_A_combo = Combobox(self,  width=30,state = "readonly")
        self.scenario_A_combo.grid(row=2, column=1,rowspan=1, sticky='nws',padx=5)

        self.loaded_scenarios_B = Label(self, text = "Compare with : ")
        self.loaded_scenarios_B.grid(row=3, column=0, columnspan=1, sticky='w')
        self.scenario_B_combo = Combobox(self,  width=30,state = "readonly")
        self.scenario_B_combo.grid(row=3, column=1,rowspan=1,columnspan=1, sticky='nw',padx=5)

        # initialise with an empty option
        self.scenario_A_combo['values'] = (*self.scenario_A_combo['values'], " ")
        self.scenario_B_combo['values'] = (*self.scenario_B_combo['values'], " ")
        ...
        for item in self.scenario_data.get_loaded_scenarios():
            ...
            for r in s_data["runs"]:
                ...
                if(r["labeled"] == True):
                    if item not in  self.scenario_A_combo['values']:
        ...
        self.compare_button_frame.grid(row=5, column=0)
        ...
        self.compare_scenarios_box_plot_btn.grid(row=5, column=0,sticky='esn',pady=5 )
        ...
        self.compare_scenarios_mean_diff_btn.grid(row=5, column=1,sticky='esn',pady=5 )
    ...
    def on_first_scenario_select(self, event):
        ...
        #if a value in position 0 remove it
        if(len(self.scenarios_to_compare)>0):
            self.scenarios_to_compare.pop(0)
        if(first_selected_scenario != " "):
            self.scenarios_to_compare.insert(0, first_selected_scenario)
    ...
    def on_second_scenario_select(self, event):
        ...
        if(len(self.scenarios_to_compare)>1):
            self.scenarios_to_compare.pop(1)
            if(second_selected_scenario != " "):
                self.scenarios_to_compare.insert(1,second_selected_scenario)
                self.compare_scenarios_mean_diff_btn.configure(state="active")
        elif(len(self.scenarios_to_compare) == 1):
            if(second_selected_scenario != " "):
                self.scenarios_to_compare.insert(1,second_selected_scenario)
                self.compare_scenarios_mean_diff_btn.configure(state="active")
        if(len(self.scenarios_to_compare) != 2):
            self.compare_scenarios_mean_diff_btn.configure(state="disabled")

        
        print(self.scenarios_to_compare)